        ...
        try {
          ...
          if (updateError) {
            ...
            try {
              ...
              const { data: userCreds, error: credsError } = await adminClient.rpc(
                'reset_user_password',
                { 
                  user_id: userData.id,
              ...
              if (credsError) {
                ...
                throw new Error('Failed to update password: ' + credsError.message);
              }
              
              // Password reset successful, now mark token as used
              const { data: markUsedResult, error: markUsedError } = await adminClient.rpc(
                'use_reset_token',
                { p_email: normalizedEmail, p_token: token }
              );
