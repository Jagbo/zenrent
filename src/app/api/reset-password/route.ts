...
async function storeResetToken(email: string, token: string) {
  ...
  try {
    ...
    if (userError) {
      ...
      const { data: directData, error: directError } = await adminClient.rpc('execute_sql', {
      ...
      if (directError || !directData) {
        ...
        const { data: rawQueryData, error: rawQueryError } = await adminClient.rpc('query_auth_user_by_email', {
          user_email: email
        });
  ...
  try {
    ...
    if (!tokenStored) {
      ...
      await supabase.auth.resetPasswordForEmail(normalizedEmail, {
        redirectTo: redirectUrl || `${process.env.NEXT_PUBLIC_SITE_URL}/reset-password`,
      });
    } else {
