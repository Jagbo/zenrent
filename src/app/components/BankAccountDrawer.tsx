...
export const BankAccountDrawer: React.FC<BankAccountDrawerProps> = ({ 
  ...
  useEffect(() => {
    ...
    const createLinkToken = async () => {
      try {
        ...
        setError(error.message || 'Failed to create Plaid Link token. Please try again.');
      } finally {
        setIsLoading(false);
      }
    };
  ...
  const handlePlaidSuccess = async (publicToken: string) => {
    try {
      ...
      if (!response.ok) {
        const errorMsg = data.details?.error_message || data.error || 'Failed to exchange token';
      ...
      return (
        <div className="space-y-6">
          <div className="space-y-2">
            ...
            <p className="text-sm text-gray-500">There was a problem connecting your bank account</p>
          </div>
