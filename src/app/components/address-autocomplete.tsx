...
export function AddressAutocomplete({ onAddressSelect, addressLine1, className = '' }: AddressAutocompleteProps) {
  ...
  const fetchAddressDetails = async (id: string) => {
    try {
      ...
      setError(error instanceof Error ? error.message : 'Failed to fetch address details');
    }
  };
              ...
              suggestions.map((result, index) => (
                ...
                >
                  ...
                  {result.locationsummary && (
                    <span className="text-gray-500 ml-1">{result.locationsummary}</span>
                  )}
                </li>
              ))
            ) : (
