...
export default function DirectoryPage() {
  const routesByFolder: Record<string, Route[]> = {
    ...
    'Onboarding': [
      ...
      { path: '/onboarding/landlord/personal-profile', name: 'Personal Profile' },
      ...
      <div className="space-y-8">
        {Object.entries(routesByFolder).map(([folder, routes]) => (
          <div key={folder} className="border rounded-lg p-4">
            <h2 className="text-xl font-semibold mb-4 text-gray-800">{folder}</h2>
              ...
              {routes.map((route) => (
                ...
                >
                  ...
                  <div className="text-sm text-gray-500">
                    {route.path}
                    {route.isDynamic && (
                      <span className="ml-2 text-xs text-blue-500">(dynamic route)</span>
                    )}
                  </div>
                </a>
              ))}
