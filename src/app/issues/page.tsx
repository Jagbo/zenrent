...
const Issues: NextPage<IssuePageProps> = () => {
  ...
  useEffect(() => {
    const fetchIssues = async () => {
      try {
        ...
        console.log(
          "Issues page: Received issues data:",
          issues.length,
          "issues",
        );
  ...
  const handleSubmit = async (formData: unknown) => {
    try {
      ...
      const issueData = {
        ...
        priority: (formData.priority || "Medium").toLowerCase() as
          | "Low"
          | "Medium"
          | "High",
        type: "Bug" as const,
    ...
    return (
      <Card key={issue.id} className="mb-4">
        <CardHeader>
          ...
          <CardDescription>{issue.property_id}</CardDescription>
        </CardHeader>
        <CardContent>
  ...
  const renderIssueList = (issues: Issue[]) => {
    return issues.map((issue: Issue) => (
      <div key={issue.id} className="mb-4">
        <button onClick={() => openDrawer(issue)} className="w-full text-left">
          <Card>
            <CardHeader>
              <CardTitle>{issue.title}</CardTitle>
              <CardDescription>Property: {issue.property_id}</CardDescription>
            </CardHeader>
            <CardContent>
              <div className="flex flex-col space-y-2">
                <div className="flex items-center justify-between">
  ...
  return (
    <SidebarLayout sidebar={<SidebarContent currentPath="/issues" />}>
          ...
          <Card>
            ...
            <CardContent>
              <p className="text-3xl font-bold text-yellow-600">
                {
                  issuesData.filter((issue) => issue.status === "In Progress")
                    .length
                }
              </p>
            </CardContent>
                  ...
                  {showFilterMenu && (
                    <div className="absolute top-16 right-6 bg-white border border-gray-200 rounded-md shadow-lg p-4 z-10 grid grid-cols-1 gap-4 w-72">
                      ...
                      <div className="flex justify-between mt-2">
                        ...
                        <button className="px-3 py-1.5 bg-gray-900 text-white text-sm font-medium rounded-md hover:bg-gray-800"
