...
export function downloadPropertyTemplate() {
  ...
  XLSX.utils.sheet_add_aoa(
    ...
    [
      [
        "456 High Street, Manchester M1 1AB",
        "hmo",
        "5",
        "2",
        "true",
...
export function validatePropertyData(properties: unknown[]): {
  valid: boolean;
  error?: string;
} {
  ...
  for (let i = 0; i < properties.length; i++) {
    ...
    if (!property.Address) {
      return {
        valid: false,
        error: `Property at row ${i + 2} is missing an address`,
      };
    }
    ...
    if (!property["Property Type"]) {
      return {
        valid: false,
        error: `Property at row ${i + 2} is missing a property type`,
      };
    }

    // Convert "Is HMO" string to boolean if needed
    if (property["Is HMO (true/false)"] !== undefined) {
      const isHmoValue = property["Is HMO (true/false)"]
        .toString()
        .toLowerCase();
      if (isHmoValue !== "true" && isHmoValue !== "false") {
        return {
          valid: false,
          error: `Property at row ${i + 2} has invalid "Is HMO" value. Please use "true" or "false"`,
        };
      }
    }
  }
...
export function preparePropertyDataForSubmission(rawData: unknown[]): unknown[] {
  return rawData.map((property) => ({
    ...
    isHmo:
      typeof property["Is HMO (true/false)"] === "boolean"
        ? property["Is HMO (true/false)"]
        : String(property["Is HMO (true/false)"] || "").toLowerCase() ===
          "true",
