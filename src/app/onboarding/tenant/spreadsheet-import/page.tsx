...
import { ArrowUpTrayIcon, CheckIcon, DocumentArrowDownIcon, ExclamationCircleIcon } from '@heroicons/react/24/outline';
  ...
  const processSpreadsheetData = async (data: any[]) => {
    try {
      ...
      if (!propertyData.id || !propertyData.address) {
        throw new Error('Missing property information. Please complete property setup first.');
      }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (propertyData.isHmo) {
        ...
        const tenants = processedData.rooms.map((room: any) => ({
          ...
          depositRegistrationDate: room.tenancyDetails.depositRegistrationDate || '',
          depositRegistrationRef: room.tenancyDetails.depositRegistrationRef || ''
        }));
        
        // Store tenants in localStorage
        localStorage.setItem('tenants', JSON.stringify(tenants));
        localStorage.setItem('tenancyData', JSON.stringify({
        ...
        const tenants = processedData.tenants.map((tenant: any) => ({
      ...
      setUploadError(error instanceof Error ? error.message : 'There was an error processing your tenant data. Please check your file format and try again.');
      setUploading(false);
      setProcessing(false);
    }
  };
  ...
  const downloadTemplate = () => {
    ...
    if (propertyData.isHmo) {
      ...
      XLSX.utils.sheet_add_aoa(worksheet, [[
        'Room Number*', 'First Name*', 'Last Name*', 'Email*', 'Phone Number',
        'Agreement Type*', 'Tenancy Term*', 'Start Date*', 'End Date',
        'Has Break Clause', 'Break Clause Details', 
        'Rent Amount*', 'Rent Frequency*', 'Rent Due Day*', 'Payment Method',
        'Deposit Amount', 'Deposit Scheme', 'Deposit Registration Date', 'Deposit Registration Reference'
      ]], { origin: 'A1' });
      
      // Add sample data
      XLSX.utils.sheet_add_aoa(worksheet, [[
        '1', 'John', 'Doe', 'john.doe@example.com', '07700 900123',
        'ast', 'fixed', '01/06/2023', '31/05/2024',
        'true', 'After 6 months with 2 months notice',
        '550', 'monthly', '1', 'bank-transfer',
        '550', 'deposit-protection-service', '02/06/2023', 'DPS12345678'
      ]], { origin: 'A2' });
    } else {
      ...
      XLSX.utils.sheet_add_aoa(worksheet, [[
        'John', 'Doe', 'john.doe@example.com', '07700 900123',
        'ast', 'fixed', '01/06/2023', '31/05/2024',
        'true', 'After 6 months with 2 months notice',
        '1200', 'monthly', '1', 'bank-transfer',
        '1200', 'deposit-protection-service', '02/06/2023', 'DPS12345678'
      ]], { origin: 'A2' });
      
      // Add sample data for second tenant
      XLSX.utils.sheet_add_aoa(worksheet, [[
        'Jane', 'Smith', 'jane.smith@example.com', '07700 900456',
        'ast', 'fixed', '01/06/2023', '31/05/2024',
        'true', 'After 6 months with 2 months notice',
        '1200', 'monthly', '1', 'bank-transfer',
        '1200', 'deposit-protection-service', '02/06/2023', 'DPS12345678'
    ...
    XLSX.writeFile(workbook, propertyData.isHmo ? 'hmo-tenant-template.xlsx' : 'tenant-template.xlsx');
  };
      ...
      <div className="divide-y divide-gray-900/10">
        ...
        <div className="grid grid-cols-1 gap-x-8 gap-y-8 py-8 md:grid-cols-3">
          <div className="px-4 sm:px-0">
            <h2 className="text-base/7 font-semibold text-gray-900">Import Tenants</h2>
            ...
            {propertyData.address && (
              <div className="mt-4 p-3 bg-gray-50 border border-gray-200 rounded-lg">
                ...
                <p className="text-xs text-gray-500">
                  {propertyData.propertyType} • {propertyData.isHmo ? 'HMO' : 'Standard'} • {propertyData.bedrooms} {parseInt(propertyData.bedrooms) > 1 ? 'Bedrooms' : 'Bedroom'}
                </p>
              </div>
            )}
          </div>
            ...
            <div className="px-4 py-4 sm:p-6">
              <div className="grid max-w-2xl grid-cols-1 gap-x-6 gap-y-8">
                ...
                <div className="border-b border-gray-900/10 pb-6">
                  <h2 className="text-base/7 font-semibold text-gray-900">Download Template</h2>
                  <p className="mt-1 text-sm/6 text-gray-600">
                    Start by downloading our template file to ensure your tenant data is formatted correctly.
                    {propertyData.isHmo ? ' This template is customized for HMO properties.' : ''}
                  </p>
                ...
                {/* File Upload */}
                <div className="border-b border-gray-900/10 pb-6">
                  <h2 className="text-base/7 font-semibold text-gray-900">Upload Your File</h2>
                  <p className="mt-1 text-sm/6 text-gray-600">
                  ...
                  <form onSubmit={handleSubmit} className="mt-4">
                    <div className="mt-2 flex justify-center rounded-lg border border-dashed border-gray-900/25 px-6 py-10">
                      <div className="text-center">
                        <ArrowUpTrayIcon className="mx-auto size-12 text-gray-300" aria-hidden="true" />
                        ...
                        <p className="text-xs leading-5 text-gray-600">Excel or CSV up to 5MB</p>
                ...
                {/* Import Guidelines */}
                <div className="border-b border-gray-900/10 pb-6">
                  <h2 className="text-base/7 font-semibold text-gray-900">Import Guidelines</h2>
                  <div className="mt-2 text-sm text-gray-600">
                    <p>For successful tenant imports:</p>
                    <ul className="mt-2 list-disc pl-5 space-y-1">
                      <li>Complete all required fields (marked with * in the template)</li>
                      <li>Use the exact format shown in the template examples</li>
                      {propertyData.isHmo && (
                        <li>Include room numbers for each tenant in HMO properties</li>
                      )}
                      <li>For contact information, ensure phone numbers and emails are valid</li>
                      <li>For lease dates, use DD/MM/YYYY format</li>
                      {!propertyData.isHmo && (
                        <li>For standard properties, list each co-tenant in a separate row</li>
                      )}
                      <li>For agreement types, use: ast, non-ast, company-let, student, or other</li>
                      <li>For tenancy terms, use: fixed or periodic</li>
                      <li>For rent frequency, use: weekly, monthly, quarterly, or annually</li>
                    </ul>
                  </div>
                </div>
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      ...
                      <p className="mt-1 text-sm text-gray-600">
                        Your tenant data is securely stored and processed in accordance with our 
                        <a href="#" className="text-indigo-600 hover:text-indigo-500"> Privacy Policy</a> and GDPR requirements. 
                        We use industry-standard encryption and security measures to protect this sensitive information.
                      </p>
                    </div>
                  </div>
                </div>
