...
export const getPropertyIssues = async (
  propertyId: string,
): Promise<IIssue[]> => {
  try {
...
export const getIssueDetails = async (
  issueId: string,
): Promise<IIssueWithDetails | null> => {
  try {
...
export const addIssueComment = async (
  comment: Partial<IIssueComment>,
): Promise<IIssueComment | null> => {
  try {
    const { data, error } = await supabase
      .from("issue_comments")
      .insert([comment])
...
export const uploadIssueMedia = async (
  ...
  try {
    ...
    const {
      data: { publicUrl },
    } = supabase.storage.from("issue-media").getPublicUrl(fileName);
...
const convertToUIIssue = (issue: unknown, properties?: unknown[]) => {
...
export const getAllIssues = async (): Promise<any[]> => {
  try {
    ...
    if (process.env.NODE_ENV === "development") {
      ...
      if (!userId) {
        console.log(
          "No authenticated user found in development mode, returning sample data",
        );
        return getSampleIssues();
      }
      console.log(
        "User found in development mode, attempting to fetch real data",
      );
    }
    ...
    if (userId) {
      ...
      if (!properties || properties.length === 0) {
        ...
        if (process.env.NODE_ENV === "development") {
          console.log(
            "Development mode: returning sample data due to no properties",
          );
          return getSampleIssues();
        }
        return [];
